...
// Place this file at: functions/_middleware.js in your repo
// Set the environment variable DASHBOARD_PASSWORD in Cloudflare Pages settings
...
.logo-fallback{font-size:22px;font-weight:700;color:#1a1d26}
...
<body>
<div class="card">
<div class="logo"><div class="logo-fallback">amino<span>sense</span></div></div>
...
function hashPassword(password) {
  // Simple HMAC-style hash using the password itself as a token
  // This isn't meant to be cryptographically bulletproof — the real
  // security comes from the cookie being httpOnly + secure + sameSite
  let hash = 0;
...
export async function onRequest(context) {
  ...
  // Allow static assets (images, fonts, etc.) through without auth
  // so the login page logo can load if you reference it
  const ext = url.pathname.split('.').pop().toLowerCase();
  const publicExts = ['png', 'jpg', 'jpeg', 'svg', 'ico', 'woff', 'woff2', 'ttf'];
  ...
  // If no password is configured, let everything through (dev mode)
  if (!PASSWORD) {
    return next();
  }
  ...
  if (request.method === "POST") {
    ...
    if (submitted === PASSWORD) {
      // Correct password — set cookie and redirect to same page
